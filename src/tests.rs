...
fn test_texture_create_valid() {
    let result = Ktx2Texture::create(256, 256, 1, 1, 1, 1, 37); // VK_FORMAT_R8G8B8A8_UNORM
    assert!(result.is_ok());
    ...
    assert_eq!(texture.vk_format(), 37);
}

#[test]
fn test_texture_create_invalid_dimensions() {
    // Zero width should fail
    let result = Ktx2Texture::create(0, 256, 1, 1, 1, 1, 37);
    assert!(result.is_err());
    assert_eq!(result.unwrap_err(), Error::InvalidValue);

    // Zero height should fail
    let result = Ktx2Texture::create(256, 0, 1, 1, 1, 1, 37);
    assert!(result.is_err());
    assert_eq!(result.unwrap_err(), Error::InvalidValue);

    // Zero depth should fail
    let result = Ktx2Texture::create(256, 256, 0, 1, 1, 1, 37);
    assert!(result.is_err());
    assert_eq!(result.unwrap_err(), Error::InvalidValue);
}
...
fn test_texture_create_excessive_dimensions() {
    ...
    let result = Ktx2Texture::create(100000, 100000, 1, 1, 1, 1, 37);
    assert!(result.is_err());
    assert_eq!(result.unwrap_err(), Error::InvalidValue);
}
...
fn test_texture_create_cubemap() {
    let result = Ktx2Texture::create(256, 256, 1, 1, 6, 1, 37); // Cubemap has 6 faces
    assert!(result.is_ok());
...
fn test_texture_create_array() {
    let result = Ktx2Texture::create(256, 256, 1, 8, 1, 1, 37); // Array with 8 layers
    assert!(result.is_ok());
...
fn test_texture_create_mipmapped() {
    let result = Ktx2Texture::create(256, 256, 1, 1, 1, 9, 37); // 9 mip levels for 256x256
    assert!(result.is_ok());
...
fn test_texture_create_3d() {
    let result = Ktx2Texture::create(128, 128, 64, 1, 1, 1, 37); // 3D texture
    assert!(result.is_ok());
...
fn test_texture_properties() {
    let texture = Ktx2Texture::create(512, 256, 1, 4, 1, 8, 37).unwrap();
...
fn test_texture_compression_status() {
    let texture = Ktx2Texture::create(256, 256, 1, 1, 1, 1, 37).unwrap();
...
fn test_texture_set_image_data_empty() {
    let mut texture = Ktx2Texture::create(256, 256, 1, 1, 1, 1, 37).unwrap();
    let empty_data: &[u8] = &[];
...
fn test_texture_extreme_small_size() {
    let result = Ktx2Texture::create(1, 1, 1, 1, 1, 1, 37);
    assert!(result.is_ok());
...
fn test_texture_power_of_two_sizes() {
    ...
    for size in sizes {
        let result = Ktx2Texture::create(size, size, 1, 1, 1, 1, 37);
...
fn test_texture_non_power_of_two_sizes() {
    ...
    for (width, height) in sizes {
        let result = Ktx2Texture::create(width, height, 1, 1, 1, 1, 37);
